...
(use-package consult
  ...
  (consult-narrow-key "?")
  ;; 末尾 / (ディレクトリ) をプレビュー対象外に。recentf-ext が recentf に
  ;; 記録したディレクトリをプレビューすると dirvish が起動し、既存の
  ;; dired バッファを kill して候補が dead buffer になるため
...
(use-package kreplace
  :ensure (kreplace
           :url "https://github.com/ichibeikatura/kreplace")
  :defer t
...
(use-package imenu-list
  ...
  :config
  ;; imenu-list は *Ilist* のモードラインを素の形式で上書きするため、
  ;; doom-modeline のバー（高さ確保用）が無くなり他の窓と高さがずれる。
